...
def predictor(game_file):
    ...
    features = np.array(data['Month Index'].tolist()).reshape(-1, 1)
    labels = np.array(data['Avg. Players'].tolist())
...
def dead_or_not(game_file):
    ...
    total = len(data['Month'])
    data['Month Index'] = pd.DataFrame([x for x in range(total, 0, -1)])
    most_player = np.max(data['Avg. Players'])
    ...
    plt.title("Predicting # of Concurrent Players In The Future")
    ...
    for x in prediction:
        if x < death_level:
            return 'In {} months, {} playerbase will be considered' \
                'dead'.format(future_months, game_file[5:-4])
        else:
            future_months += 1
...
def average_predictor(game_file):
    ...
    if average_f6 < avg_next_6:
        return "This game, {}, is increasing in popularity"\
            " by {}%".format(game_file[5:-4], (avg_next_6/average_f6)-1)
    else:
        return "This game, {}, appears not to be increasing"\
            " in popularity".format(game_file[5:-4])


def linear_predictor(game_file):
    data = pd.read_csv(game_file)
    total = len(data['Month'])
    data['Month Index'] = pd.DataFrame([x for x in range(total, 0, -1)])
    actual_fy = data[-12:-7]
    ...
    plt.title("Popularity Predictor")
    ...
    if average_fy < average_predicted:
        return "This game, {}, is increasing in popularity"\
            " by {}%".format(game_file[5:-4], (average_predicted/average_fy)-1)
    else:
        return "This game, {}, appears not to be increasing in"\
            " popularity".format(game_file[5:-4])


def main():
    predictor('data/csgo.csv')
    print(dead_or_not('data/csgo.csv'))
    print(average_predictor('data/csgo.csv'))
    print(linear_predictor('data/csgo.csv'))
